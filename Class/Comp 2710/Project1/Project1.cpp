...
class scores {
    ...
    void readScore(){
        ...
        string name;
        int i = 1;
        while(reader>>name && i < MAX_SCORES){
            scoreL[i].name = name;
            reader>>scoreL[i].point;
            counter++;
            i++;
        }
    }
...
class events {
    ...
    void riddle(player &pL){
        ...
        do{
            ...
            if(choice == p ->correct){
                mod = 1;
                message = "\"Lucky guess... I guess I should reward you or something...\"\n";
            }
            else{
...
class game{
    ...
    int play(){
        ...
        while(score > 0 && dist != NUM_SPACES){
            ...
            cout<<" Time left "<<p1.time<<"."<<endl<<endl;
            turn();
            score = p1.intell * p1.money * p1.time;
        }
        if(score < 0){
